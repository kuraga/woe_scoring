...
from woe_scoring.core.binning.functions import (
    _merge_bins_chi, _extract_bin_by_chi2, _merge_bins_iv,
    _extract_bin_by_iv, _merge_bins_min_pct, _calc_stats,
    _bin_bad_rates, _calc_max_bins, prepare_data, find_cat_features,
    _cat_binning, cat_processing, _num_binning, num_processing,
    _refit_woe_dict, refit
)
...
class TestBinningFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.cat_data = pd.DataFrame({
            ...
            'imbalanced': np.random.choice(['Rare', 'Common'], n_samples, p=[0.1, 0.9])
        })
        ...
        self.test_bins: List[Dict] = [
            {"bad": 5, "total": 20, "bad_rate": 0.25, "woe": 0.5},
            {"bad": 10, "total": 20, "bad_rate": 0.5, "woe": 0.0},
            {"bad": 15, "total": 20, "bad_rate": 0.75, "woe": -0.5},
            {"bad": 18, "total": 20, "bad_rate": 0.9, "woe": -1.0}
        ]

        # Calculate overall rate for tests
        total_bad = sum(b["bad"] for b in self.test_bins)
        total_count = sum(b["total"] for b in self.test_bins)
        self.overall_rate = total_bad / total_count
    ...
    def test_find_cat_features(self):
        ...
        cat_features = find_cat_features(x=test_data, feature_names=feature_names, cat_features_threshold=5)
    ...
    def test_calc_stats(self):
        ...
        all_bad = test_df['target'].sum()  # Sum of all bad cases
        all_good = len(test_df['target']) - all_bad  # Total good cases
        bins = ['A', 'B', 'C']  # List of unique values

        # Call _calc_stats with correct parameters
        result = _calc_stats(test_df['feature'].values, test_df['target'].values, 0, all_bad, all_good, bins, True)
    ...
    def test_bin_bad_rates(self):
        ...
        self.assertEqual(bin2['bad_rate'], 0.5)
    ...
    def test_merge_bins_chi(self):
        ...
        # Calculate bad rates
        bad_rates = [
            {"bin": 1, "bad": 1, "total": 1, "bad_rate": 1.0},
            {"bin": 2, "bad": 0, "total": 1, "bad_rate": 0.0},
            {"bin": 3, "bad": 1, "total": 1, "bad_rate": 1.0},
            {"bin": 4, "bad": 0, "total": 1, "bad_rate": 0.0}
        ]

        bins = [1, 2, 3, 4]

        bad_rates_result, bins_result = _merge_bins_chi(bad_rates, bins, self.overall_rate)

        # Check that the result is a list with one less bin
        self.assertIsInstance(bad_rates_result, list)
        self.assertEqual(len(bad_rates_result), len(bad_rates) - 1)
    ...
    def test_merge_bins_iv(self):
        ...
        # Calculate bad rates
        bad_rates = [
            {"bin": 1, "bad": 1, "total": 1, "bad_rate": 1.0, "woe": 0.5},
            {"bin": 2, "bad": 0, "total": 1, "bad_rate": 0.0, "woe": 0.0},
            {"bin": 3, "bad": 1, "total": 1, "bad_rate": 1.0, "woe": -0.5},
            {"bin": 4, "bad": 0, "total": 1, "bad_rate": 0.0, "woe": -1.0}
        ]

        bins = [1, 2, 3, 4]

        # Test bin merging directly with the bad_rates and bins
        bad_rates_result, bins_result = _merge_bins_iv(bad_rates, bins)
    ...
    def test_merge_bins_min_pct(self):
        ...
        # Calculate bad rates
        bad_rates = [
            {"bin": 1, "bad": 1, "total": 5, "bad_rate": 0.2, "pct": 0.02},
            {"bin": 2, "bad": 2, "total": 10, "bad_rate": 0.2, "pct": 0.04},
            {"bin": 3, "bad": 5, "total": 100, "bad_rate": 0.05, "pct": 0.4},
            {"bin": 4, "bad": 2, "total": 135, "bad_rate": 0.01, "pct": 0.54}
        ]

        bins = [1, 2, 3, 4]

        min_pcnt = 0.05
        bad_rates_result, bins_result = _merge_bins_min_pct(bad_rates, bins, min_pcnt)

        # Check that small bins are merged
        self.assertIsInstance(bad_rates_result, list)
        self.assertLess(len(bad_rates_result), len(bad_rates))

        # Verify no bin has pct < min_pcnt
        for bin_dict in bad_rates_result:
    ...
    def test_cat_processing(self):
        ...
        for bin_info in result['low_card']:
            ...
            self.assertIn('bad', bin_info)
            self.assertIn('bad_rate', bin_info)
